...
pub fn simplify(arg: ParsingResult) -> ParsingResult {
    match arg {
        ParsingResult::Alt {
            ...
            let mut head_item = alt_args.pop_front().unwrap();
            if let AltArg::Star(arg) = head_item {
                head_item = AltArg::Concat {
                    args: vec![ConcatArg::Star(arg)],
                    accepts_empty: true,
                };
            }

            if let AltArg::Concat { args: mut first_args, accepts_empty } =
                head_item
            {
                        ...
                        alt_args.iter().all(|item| match item {
                            AltArg::Concat { args, accepts_empty } => {
                                if head_trim_number == args.len() {
                                    ...
                                    item_repr
                                        == args[head_trim_number].to_string()
                                }
                            }
                            AltArg::Star(_) => false,
                        });
                    ...
                    AltArg::Concat { mut args, accepts_empty } => {
                        if args.len() == 1 {
                            ...
                            match first {
                                ...
                                ConcatArg::Char(c) => t.push(AltArg::Concat {
